...
def callback(topic, msg):
    ...
    global read_mqtt_message
    
    read_mqtt_message = [] # Wipe the previous message
    message = msg.decode("utf-8")
    read_mqtt_message.append(message)
...
def MQTT_broker_connect():
    """
    Connects to MQTT broker and generate the client MQTT object.
...
def get_mqtt_message():
    """
    Recovers the message publish in the MQTT topic to which it had previously subscribed.
    
    Inputs:
    - None
    Returns:
    - Last MQTT message publish
    """
    
    return read_mqtt_message
